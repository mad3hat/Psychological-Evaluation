...
type Questions struct {
	...
	B         string    `orm:"column(b);null"`
	C         string    `orm:"column(c);null"`
	D         string    `orm:"column(d);null"`
...
func init() {
	orm.RegisterModel(new(Questions))
}

// AddQuestions insert a new Questions into database and returns
// last inserted Id on success.
func AddQuestions(que string, a string, b string, c string, d string) (err error) {
	o := orm.NewOrm()
	//o.Using("default")
...
func GetAllQuestions(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []Questions, err error) {
	o := orm.NewOrm()
...
// UpdateQuestions updates Questions by Id and returns error if
// the record to be updated doesn't exist
func UpdateQuestionsById(m *Questions, body string, a string, b string, c string, d string) (err error) {
	o := orm.NewOrm()
	v := Questions{Id: m.Id}   //copy m to v
	...
	v.C = c
	v.D = d
	if _, err = o.Update(&v, "Body", "A", "B", "C", "D"); err != nil {
		fmt.Println("更新失败！")
	}
	return
}
...
func DeleteQuestions(id int) (err error) {
	o := orm.NewOrm()
	v := Questions{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Questions{Id: id}); err == nil {
