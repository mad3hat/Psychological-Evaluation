...
func AddArticle(title string, text string) (err error) {
	o := orm.NewOrm()
	//o.Using("default")
	article := new(Article)
	article.Topic = title
...
func GetAllArticle(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []Article, err error) {
	o := orm.NewOrm()
...
func UpdateArticleById(m *Article, topic string, body string) (err error) {
	o := orm.NewOrm()
	v := Article{Id: m.Id}   //copy m to v
	// ascertain id exists in the database
	v.Topic = topic
	v.Body = body
	if _, err = o.Update(&v, "Topic", "Body"); err != nil {
		fmt.Println("更新失败！")
	}
	return
}
